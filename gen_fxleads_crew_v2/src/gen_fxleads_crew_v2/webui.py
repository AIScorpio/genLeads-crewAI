...
from gen_fxleads_crew_v2.crew import GenFxleadsCrewV2
from RateLimitErrorHandler import call_crew_with_retry
import time

class FXLeadsGenUI:

    def _generate_leads(self, industry, material, headquatar, region, expected_num, offering):
        # if int(expected_num) > 0:
        #     search_num = expected_num * 2
        # else:
        #     search_num = 10
        ...
        leads_details = {
            'industry': industry,
            'material': material,
            'headquatar': headquatar,
            'region': region,
            # 'search_num': search_num,
            'expected_num': expected_num,
            "offering": offering,  # NDF : FX Forward
        }
        # return GenFxleadsCrewV2().crew().kickoff(inputs=leads_details)
        return call_crew_with_retry(GenFxleadsCrewV2().crew(), leads_details)
    ...
    def sidebar(self):
        with st.sidebar:
            ...
            st.text_input("headquatar", key="headquatar", placeholder="Enter Headquatar e.g. New York")
    ...
    def render(self):
        ...
        if "industry" not in st.session_state:
            st.session_state["industry"] = "Reflective Conductive Coatings"
        if "material" not in st.session_state:
            st.session_state["material"] = "silver"
        if "headquatar" not in st.session_state:
            st.session_state["headquatar"] = "Europe"
        if "region" not in st.session_state:
            st.session_state["region"] = "China"
        if "expected_num" not in st.session_state:
            st.session_state["expected_num"] = 10
